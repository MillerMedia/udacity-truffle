...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(await instance.symbol(), 'MST');
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user1 = accounts[1];
    let starId = 10;
    await instance.createStar('First Star', starId, {from: user1});

    // 2. Call your method lookUptokenIdToStarInfo
    // 3. Verify if you Star name is the same
    assert.equal(await instance.lookUptokenIdToStarInfo.call(starId), "First Star", "Name method not returning correct value.");
});